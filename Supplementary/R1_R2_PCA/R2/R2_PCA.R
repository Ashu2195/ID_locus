tiff("R2_1_2.tiff", width = 7500, height = 5000, res = 350)
C <- read.table(file="genolike_R2.cov.cov")
m<-as.matrix(C)
e <- eigen(m, symm=TRUE)
...
box(lwd=3)
legend("bottomright",legend=c("Black Bone Chicken", "Non Black Native Chicken","Commercial Chicken","Red Junglefowl", "Green Junglefowl","Ceylon Junglefowl","Grey Junglefowl" ),col=c("black","blue","orange","red","green","magenta","grey"),pch=c(16, 16, 16,16, 16, 16,16),cex=2,pt.cex=2)
dev.off()


tiff("R2_1_3.tiff", width = 7500, height = 5000, res = 350)
C <- read.table(file="genolike_R2.cov.cov")
m<-as.matrix(C)
e <- eigen(m, symm=TRUE)
e$val <- e$val/sum(e$val)
...
tiff("R2_2_3.tiff", width = 7500, height = 5000, res = 350)
C <- read.table(file="genolike_R2.cov.cov")
m<-as.matrix(C)
e <- eigen(m, symm=TRUE)
e$val <- e$val/sum(e$val)
